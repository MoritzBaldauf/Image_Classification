...
class ImagesDataset(Dataset):
    def __init__(
        ...
        # Look for CSV file in the current directory and parent directory
        csv_files = glob(path.join(image_dir, "*.csv"))
        if not csv_files:
            parent_dir = os.path.dirname(image_dir)
            csv_files = glob(path.join(parent_dir, "*.csv"))

        if not csv_files:
            raise ValueError(f"No CSV file found in {image_dir} or its parent directory")

        class_filepath = path.abspath(csv_files[0])
        print(f"Using class file: {class_filepath}")
    ...
    def load_classnames(class_filepath: str):
        ...
        filenames_classnames = [line.strip().split(';') for line in lines]
        classnames = sorted(set(classname for _, classname in filenames_classnames))
        classnames_to_ids = {name: idx for idx, name in enumerate(classnames)}
        return filenames_classnames, classnames_to_ids

    def __len__(self):
        return len(self.filenames_classnames)

    def __getitem__(self, index):
        filename, classname = self.filenames_classnames[index]
        img_path = path.join(self.image_dir, filename)
        ...
        return resized_image, classid, classname, filename
